...
// Safe font injection (only once)
if (!document.getElementById("f1-fonts")) {
  const link = document.createElement("link");
  link.id = "f1-fonts";
  link.rel = "stylesheet";
  link.href =
    "https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Share+Tech+Mono&family=Rajdhani:wght@300;400;600;700&display=swap";
  document.head.appendChild(link);
}

// Relative path — Vite proxy forwards to http://localhost:8001 (docker-compose port)
const API = "";
...
function teamColor(team = "") {
  ...
  for (const [k, v] of Object.entries(TEAM_COLOR)) {
...
function ProbChart({ data, w = 480, h = 120 }) {
  ...
  const range = mx - mn || 1; // avoid division by zero
  const x = (i) => (i / (data.length - 1)) * w;
  const y = (v) => h - ((v - mn) / range) * h;
  const line = data.map((_, i) => `${x(i)},${y(vals[i])}`).join(" ");
  const area =
    `M${x(0)},${y(vals[0])} ` +
    data.map((_, i) => `L${x(i)},${y(vals[i])}`).join(" ") +
    ...
    results.length > 0
      ? results
          ...
          .filter(
            (r) =>
              r.commentary_llm &&
              !r.commentary_llm.startsWith("Erreur") &&
              r.commentary_llm !== "Commentaire en attente..."
          )
          .map((r) => ({
            icon: "▶",
            c: "#e8001d",
            text: `LAP ${r.lap} — ${r.commentary_llm.slice(0, 90)}...`,
          }))
      : TICKER_DEFAULT;

  // Need at least 1 item for the ticker to work
  const safeItems = items.length > 0 ? items : TICKER_DEFAULT;
  const content = [...safeItems, ...safeItems, ...safeItems];
...
function CommentItem({ persona, text, lap }) {
  ...
  if (!p) return null; // Guard against unknown persona
    ...
    <div style={{ padding: "14px 0", borderBottom: "1px solid #141414" }}>
      ...
      <p
        style={{
          ...
          lineHeight: 1.65,
          margin: 0,
          color: isErr || isPend ? "#2a2a2a" : "#999",
          fontStyle: isPend ? "italic" : "normal",
        }}
      >
...
function IndCard({ name }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ fontSize: 14, opacity: 0.6, color: c.ac }}>{c.icon}</span>
    </div>
  );
}
...
export default function F1Dashboard() {
  ...
  // Use ref to track total laps expected (to compute real %)
  const expectedLaps = useRef(57);

  const cur = results.find((r) => r.lap === selLap) ?? results[results.length - 1];

  // Auto-scroll commentary to bottom when new data arrives
  useEffect(() => {
    if (cmtRef.current) cmtRef.current.scrollTop = cmtRef.current.scrollHeight;
  }, [selLap, results]);

  // Auto-scroll lap chips to selected chip
  useEffect(() => {
  ...
  // FIX: poll uses a ref to avoid stale closure on `results`
  ...
  const poll = useCallback(
    async (id) => {
      try {
        ...
        if (data.resultats?.length) {
          ...
          // Real progress: based on laps processed vs expected total
          const pct = Math.min(95, Math.round((data.resultats.length / expectedLaps.current) * 100));
          setProgress(pct);
          // Select first lap on first results arrival
          setSelLap((prev) => prev ?? data.resultats[0]?.lap);
        }
        ...
        if (data.status === "erreur") {
          setPhase("error");
          setErrMsg(data.message ?? "Erreur inconnue");
          clearInterval(pollRef.current);
        }
      } catch (e) {
        setPhase("error");
    ...
    [] // safe: no stale closure issues with refs
  );
  ...
  const indicators = cur?.indicators
    ? Object.entries(cur.indicators)
        .filter(([, v]) => v)
        .map(([k]) => k)
    : [];
  const withComment = results.filter(
    (r) =>
      r.commentary_llm &&
      !r.commentary_llm.startsWith("Erreur") &&
      r.commentary_llm !== "Commentaire en attente..."
  ...
  const statusText =
    phase === "processing" ? "ANALYZING" : phase === "done" ? "RACE COMPLETE" : "PRE-RACE";
  const statusColor =
    phase === "processing" ? "#e8001d" : phase === "done" ? "#00e676" : "#333";

  // Show last 10 results, or only selected lap
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          >
            <span style={{ fontFamily: "'Orbitron'", fontWeight: 900, fontSize: 11, color: "#fff" }}>
              F1
            </span>
          </div>
          <span
            style={{ fontFamily: "'Orbitron'", fontWeight: 700, fontSize: 13, letterSpacing: 2, color: "#fff" }}
          >
            LIVE AI COMMENTATOR
          </span>
        </div>
        ...
        >
          ...
          {results.length > 0 && (
            <>
              <span style={{ color: "#e8001d" }}>·</span>
              <span style={{ color: "#666" }}>LAP {cur?.lap ?? 0}</span>
            </>
          )}
        </div>
        ...
        >
          ...
          <div style={{ padding: "14px 16px", borderBottom: "1px solid #141414", flexShrink: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginTop: 6 }}>
              <span style={{ fontFamily: "'Orbitron'", fontWeight: 900, fontSize: 28, color: "#fff" }}>
                {cur?.prediction?.probability?.toFixed(0) ?? "—"}%
              </span>
              <span style={{ fontFamily: "'Share Tech Mono'", fontSize: 11, color: "#00e676" }}>
                ▲{cur ? ((cur.prediction?.probability ?? 50) - 50).toFixed(1) : "0.0"}
              </span>
            </div>
          ...
          <div style={{ padding: "12px 16px", borderBottom: "1px solid #141414", flexShrink: 0 }}>
            ...
            {drivers.slice(0, 4).map((d, i) => (
              ...
              >
                <span style={{ fontFamily: "'Share Tech Mono'", fontSize: 9, color: "#222", width: 12, flexShrink: 0 }}>
                  {d.position ?? i + 1}
                </span>
          ...
          {cur && (
            <div style={{ padding: "12px 16px", background: "#0a0a0a", flexShrink: 0 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div>
                  <div style={{ fontFamily: "'Share Tech Mono'", fontSize: 10, color: "#666" }}>
                    {cur.prediction?.probability?.toFixed(1)}% CONF.
                  </div>
                  <div style={{ fontFamily: "'Share Tech Mono'", fontSize: 9, color: "#252525", marginTop: 2 }}>
                    ≈ {cur.prediction?.position_exacte?.toFixed(2)}
                  </div>
                  <div style={{ fontFamily: "'Share Tech Mono'", fontSize: 9, color: "#252525", marginTop: 1 }}>
                    T={cur.timestamp}s
                  </div>
                </div>
              </div>
              <div style={{ height: 2, background: "#111", marginTop: 10 }}>
                <div
                  style={{
                    height: "100%",
                    width: `${cur.prediction?.probability ?? 0}%`,
                    background: "linear-gradient(90deg,#e8001d,#ffd700)",
                    transition: "width .8s",
                  }}
                />
              </div>
        ...
        >
          ...
          <div
            style={{
              ...
              maxHeight: "40vh",
              overflow: "hidden",
              outline: drag ? "2px solid #e8001d" : "none",
            }}
          ...
          >
              ...
              >
                <div
                  style={{
                    width: 48,
                    height: 48,
                    border: `2px solid ${drag ? "#e8001d" : "#1e1e1e"}`,
                    borderRadius: "50%",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                ...
                {file ? (
                  <>
                    ...
                    <div style={{ fontFamily: "'Share Tech Mono'", fontSize: 9, color: "#333", letterSpacing: 2 }}>
                  ...
                  <>
                    ...
                    >
                      Drag &amp; Drop Your Race Footage
                    </div>
                    <div style={{ fontFamily: "'Share Tech Mono'", fontSize: 10, color: "#1a1a1a", letterSpacing: 3 }}>
                      MP4 · MKV · AVI
                    </div>
                  </>
                )}
            ...
            {(phase === "uploading" || phase === "processing") && (
              ...
              >
                ...
                <div style={{ fontFamily: "'Share Tech Mono'", fontSize: 9, color: "#252525", letterSpacing: 2 }}>
                  {progress}%
                </div>
              </div>
            )}
              ...
              <>
                <div
                  style={{
                    position: "absolute",
                    inset: 0,
                    background: "radial-gradient(ellipse at 30% 50%, #0d0815 0%, #080808 70%)",
                  }}
                />
                <div
                  style={{
                ...
                >
                  <div style={{ fontFamily: "'Share Tech Mono'", fontSize: 8, color: "#333", letterSpacing: 2, marginBottom: 3 }}>
                    CURRENT FOCUS
                  </div>
                  <div style={{ fontFamily: "'Rajdhani'", fontSize: 14, fontWeight: 700, color: "#fff" }}>
                    {cur.drivers?.[0]?.name ?? "RACE ANALYSIS"}
                  </div>
                  <div style={{ fontFamily: "'Share Tech Mono'", fontSize: 9, color: "#555", marginTop: 2 }}>
                    PREDICTED P{cur.prediction?.position_predite} · {cur.prediction?.probability?.toFixed(1)}%
                  </div>
                </div>
                {indicators.length > 0 && (
                  ...
                  <div style={{ flex: 1, height: 2, background: "#1a1a1a", position: "relative" }}>
                    <div
                      style={{
                        ...
                        top: 0,
                        height: "100%",
                        width: `${(cur.lap / (expectedLaps.current || 57)) * 100}%`,
                        background: "#e8001d",
                      }}
                    />
                  </div>
          ...
          {results.length > 0 && (
            <div style={{ borderBottom: "1px solid #141414", background: "#0a0a0a", flexShrink: 0 }}>
              <div
                ref={lapRef}
                style={{ display: "flex", gap: 3, overflowX: "auto", padding: "6px 10px", scrollbarWidth: "thin" }}
              >
                {results.map((r) => {
                  const hc =
                    r.commentary_llm &&
                  ...
                  return (
                    <div
                      key={r.lap}
                      data-lap={r.lap}
                      onClick={() => setSelLap(r.lap)}
                      style={{
                        flexShrink: 0,
                        width: 32,
                        height: 32,
                        background: active ? "#e8001d" : "#111",
                        border: `1px solid ${active ? "#e8001d" : "#1a1a1a"}`,
                        display: "flex",
                        alignItems: "center",
                        justifyContent: "center",
                        cursor: "pointer",
          ...
          <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", overflow: "hidden", minHeight: 0 }}>
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8, flexShrink: 0 }}>
                <div>
                  <div style={{ fontFamily: "'Share Tech Mono'", fontSize: 9, letterSpacing: 3, color: "#e8001d", marginBottom: 2 }}>
                    VICTORY PROBABILITY
                  </div>
                  <div style={{ fontFamily: "'Share Tech Mono'", fontSize: 8, color: "#1e1e1e", letterSpacing: 1 }}>
                    MODEL: GEMINI-1.5-FLASH
                  </div>
                </div>
                <span style={{ fontFamily: "'Share Tech Mono'", fontSize: 8, color: "#1e1e1e" }}>
                  {withComment}/{results.length}
                </span>
              </div>
                ...
                <div style={{ display: "flex", gap: 12, marginTop: 6, fontFamily: "'Share Tech Mono'", fontSize: 9, flexShrink: 0 }}>
                  <span style={{ color: "#222" }}>
                    ...
                    <span style={{ color: "#e8001d" }}>
                      {Math.min(...results.map((d) => d.prediction?.probability ?? 50)).toFixed(1)}%
                    </span>
                  </span>
                  <span style={{ color: "#222" }}>
                    MAX{" "}
                    <span style={{ color: "#00e676" }}>
                      {Math.max(...results.map((d) => d.prediction?.probability ?? 50)).toFixed(1)}%
                    </span>
                  </span>
                </div>
              )}
            ...
            <div style={{ padding: "12px 14px", overflow: "auto" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, flexShrink: 0 }}>
                <div style={{ fontFamily: "'Share Tech Mono'", fontSize: 9, letterSpacing: 3, color: "#252525" }}>
                  LIVE TIMING
                </div>
                <div
                  style={{
              ...
              {drivers.slice(0, 6).map((d, i) => (
                ...
                >
                  <span style={{ fontFamily: "'Share Tech Mono'", fontSize: 9, color: "#252525", width: 12, flexShrink: 0 }}>
                    {d.position ?? i + 1}
                  </span>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontFamily: "'Share Tech Mono'", fontSize: 8, color: "#222", marginTop: 1 }}>
                      {(d.team ?? "—").toUpperCase()}
                    </div>
                  </div>
                  <span
                    style={{
                      fontFamily: "'Share Tech Mono'",
                      fontSize: 10,
                      color: d.interval === "LEADER" || i === 0 ? "#00e676" : "#444",
                      flexShrink: 0,
                    }}
                  >
                    {d.interval ?? (i === 0 ? "LEADER" : "—")}
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  background: "transparent",
                  border: "none",
                  borderBottom: tab === key ? "2px solid #e8001d" : "2px solid transparent",
                  fontFamily: "'Share Tech Mono'",
                  fontSize: 9,
                  letterSpacing: 2,
                  color: tab === key ? "#fff" : "#252525",
                  cursor: "pointer",
                  transition: "all .2s",
                  textTransform: "uppercase",
                }}
          ...
          <div ref={cmtRef} style={{ flex: 1, overflowY: "auto", padding: "0 14px" }}>
            {toShow.length === 0 ? (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%" }}>
                <span
                  style={{ fontFamily: "'Share Tech Mono'", fontSize: 10, color: "#1e1e1e", letterSpacing: 3 }}
                >
                  WAITING FOR RACE FEED...
                </span>
              </div>
            ) : (
              toShow.map((r, i) => (
                ...
                >
                  {tab === "commentary" && (
                    <>
                      ...
                      {r.commentary_rag &&
                        r.commentary_rag !== "Commentaire en attente..." &&
                        !r.commentary_rag.startsWith("Erreur") && (
                  ...
                  {tab === "strategy" && (
                    <div style={{ padding: "12px 0", borderBottom: "1px solid #141414" }}>
                      <div style={{ fontFamily: "'Share Tech Mono'", fontSize: 9, color: "#252525", marginBottom: 8 }}>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                        ...
                        {!Object.values(r.indicators ?? {}).some(Boolean) && (
                          <span style={{ fontFamily: "'Share Tech Mono'", fontSize: 9, color: "#1e1e1e" }}>
          ...
          {phase === "done" && (
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", fontFamily: "'Share Tech Mono'", fontSize: 9, color: "#252525" }}>
                ...
                <span onClick={resetAll} style={{ color: "#e8001d", cursor: "pointer", letterSpacing: 1 }}>
                  ↺ NEW RACE
                </span>
              </div>
            </div>
